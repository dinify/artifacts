    ...
    */
    ...
    locales.forEach(locale => {
      ...
      if (!exclude.includes(locale)) {
        ...
        jsonObj.forEach(entry => {
          ...
          if (translation.toLowerCase() !== entry.withContext.toLowerCase()) {
            if (entry.subs) {
              ...
              if (!translation.includes(entry.subs)) console.warn('! not substituted',  entry.key, locale, 'substitution: ', entry.subs);
        ...
        fs.writeFileSync(`./locales/${replaced}/${namespace}.json`, JSON.stringify(result, null, 2));
